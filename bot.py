...
MONGO_DB_NAME = os.environ.get("MONGO_DB_NAME", "rclone_bot").strip()
...
last_job_delete: bool = False        # delete flag the last job ran with (for /retry)
auto_restart_enabled: bool = False   # 10-min watchdog auto-restart of stopped jobs
# Defined here (before _load_config runs at import) so a value restored from
# MongoDB isn't clobbered by a later module-level assignment.
...
# ── MongoDB: config persistence + last-job tracking ─────────────
# Heroku wipes the dyno filesystem and restarts the process every ~24h, so
# all runtime config is mirrored to MongoDB whenever it changes and reloaded
# on startup. The last job's parameters are stored too, so a job interrupted
# by the restart can be resumed automatically when the dyno comes back up.
mongo_client = None
mongo_cfg = None       # config collection (single doc, _id="config")
mongo_job = None       # last-job collection (single doc, _id="last_job")
mongo_lock = threading.Lock()


def _mongo_init() -> None:
    """Connect to MongoDB if MONGO_URI is set and pymongo is installed."""
    global mongo_client, mongo_cfg, mongo_job
    if not MONGO_URI:
        log.info("MONGO_URI not set — config persistence disabled.")
        return
    if MongoClient is None:
        log.warning("pymongo not installed — config persistence disabled. "
    ...
    try:
        ...
        mongo_cfg = db["config"]
        mongo_job = db["last_job"]
        log.info(f"MongoDB connected — db={MONGO_DB_NAME!r}")
    except Exception as e:
        log.error(f"MongoDB connection failed: {e}")
        mongo_client = mongo_cfg = mongo_job = None


def _config_snapshot() -> dict:
    """Collect the current runtime config into a plain dict."""
    with state_lock:
        return {
...
def _save_config() -> None:
    """Persist the current runtime config to MongoDB (best-effort)."""
    if mongo_cfg is None:
        return
    try:
        snap = _config_snapshot()
        with mongo_lock:
            mongo_cfg.update_one(
                {"_id": "config"},
                {"$set": snap},
                upsert=True,
            )
        ...
        log.warning(f"_save_config failed: {e}")


def _load_config() -> None:
    """Load saved config from MongoDB and apply it to the runtime globals."""
    global delete_after_upload, max_size_enabled, max_size_bytes
    global BW_LIMIT, CONCURRENT_JOBS, BOARD_REFRESH_INTERVAL, auto_restart_enabled
    if mongo_cfg is None:
        return
    try:
        with mongo_lock:
            doc = mongo_cfg.find_one({"_id": "config"})
        if not doc:
            log.info("No saved config in MongoDB yet — using defaults/env.")
            return
        with state_lock:
            if isinstance(doc.get("delete_after_upload"), bool):
                delete_after_upload = doc["delete_after_upload"]
            if isinstance(doc.get("max_size_enabled"), bool):
                max_size_enabled = doc["max_size_enabled"]
            if isinstance(doc.get("max_size_bytes"), int) and doc["max_size_bytes"] > 0:
                max_size_bytes = doc["max_size_bytes"]
            if isinstance(doc.get("BW_LIMIT"), str):
                BW_LIMIT = doc["BW_LIMIT"]
            if isinstance(doc.get("CONCURRENT_JOBS"), int) and 1 <= doc["CONCURRENT_JOBS"] <= 5:
                CONCURRENT_JOBS = doc["CONCURRENT_JOBS"]
            if isinstance(doc.get("BOARD_REFRESH_INTERVAL"), (int, float)) and 2.0 <= doc["BOARD_REFRESH_INTERVAL"] <= 30.0:
                BOARD_REFRESH_INTERVAL = float(doc["BOARD_REFRESH_INTERVAL"])
            if isinstance(doc.get("auto_restart_enabled"), bool):
                auto_restart_enabled = doc["auto_restart_enabled"]
        log.info("Config restored from MongoDB.")
    except Exception as e:
        log.warning(f"_load_config failed: {e}")


def _save_last_job(remote_path: str, mode: str, do_delete: bool) -> None:
    """Record the most recent job so it can be auto-resumed after a restart."""
    if mongo_job is None:
        return
    try:
        with mongo_lock:
            mongo_job.update_one(
                {"_id": "last_job"},
                {"$set": {
        ...
        log.warning(f"_save_last_job failed: {e}")


def _mark_last_job(status: str) -> None:
    """Update the stored last-job status: 'done', 'cancelled', 'failed'."""
    if mongo_job is None:
        return
    try:
        with mongo_lock:
        ...
        log.warning(f"_mark_last_job failed: {e}")


def _get_last_job() -> dict | None:
    if mongo_job is None:
        return None
    try:
        with mongo_lock:
            return mongo_job.find_one({"_id": "last_job"})
    except Exception as e:
        log.warning(f"_get_last_job failed: {e}")
        return None


def _mongo_close() -> None:
    """Close the MongoDB connection cleanly before the process exits."""
    ...
    try:
        ...
        log.debug(f"_mongo_close failed: {e}")
...
def split_file(local_path: str, split_dir: str, user_id: int = 0) -> list[str]:
    ...
    try:
        ...
        log.error(
            f"split_file: ffprobe unavailable ({e}) — uploading {filename} as-is."
        )
        return [local_path]
    duration = 0
    try:
    ...
    while i <= parts or start_time < duration - 4:
        ...
        try:
            ...
            log.error(
                f"split_file: split command unavailable ({e}) — "
                f"uploading {filename} as-is."
            )
            return out_paths or [local_path]
        ...
        try:
            ...
            # ffprobe vanished mid-job; keep the part we produced and stop.
...
async def _run_job(status_msg: Message, files: list[str], remote_path: str,
    ...
    global last_failed, last_job_remote, last_job_mode, last_job_delete
    ...
    _save_last_job(remote_path, mode, do_delete)
    with stats_lock:
    ...
    with state_lock:
        ...
        last_job_mode = mode
        last_job_delete = do_delete
        was_cancelled = cancel_flags.get(user_id, False)

    # Record final job status for the watchdog. A job is only "done" when it
    # wasn't cancelled and produced no failures; otherwise it stays resumable.
    if was_cancelled:
        _mark_last_job("cancelled")
    elif results["failed"]:
        _mark_last_job("failed")
    else:
        _mark_last_job("done")
...
async def cmd_start(_, msg: Message):
    await msg.reply(
        ...
        "`/autorestart on|off` — auto-resume a stopped job every 10 min (needs MongoDB)\n"
...
@app.on_message(filters.command("autorestart") & auth_filter)
async def cmd_autorestart(_, msg: Message):
    global auto_restart_enabled
    if len(msg.command) < 2:
        with state_lock:
            state = "ON 🟢" if auto_restart_enabled else "OFF 🔴"
        await msg.reply(
            f"♻️ **Auto-restart is currently: {state}**\n\n"
            "When the dyno restarts (e.g. Heroku's daily ~24h cycle), the bot "
            "checks on startup whether the last job was still running and, if "
            "so, automatically resumes it — no manual `/dl` needed.\n\n"
            "Usage:\n"
            "`/autorestart on` — resume an interrupted job on next startup\n"
            "`/autorestart off` — disable auto-resume\n\n"
            "⚠️ Requires `MONGO_URI` to be configured."
        )
        return
    arg = msg.command[1].strip().lower()
    if arg == "on":
        if mongo_job is None:
            await msg.reply("❌ Auto-restart needs MongoDB. Set `MONGO_URI` and restart the bot.")
            return
        with state_lock:
            auto_restart_enabled = True
        _save_config()
        await msg.reply("♻️ **Auto-restart: ON 🟢**\nAn interrupted job will be auto-resumed when the bot restarts.")
    elif arg == "off":
        with state_lock:
            auto_restart_enabled = False
        _save_config()
        await msg.reply("♻️ **Auto-restart: OFF 🔴**\nJobs will not be auto-resumed.")
    else:
        await msg.reply("❌ Usage: `/autorestart on` or `/autorestart off`")


async def _resume_on_startup():
    """Run once after the bot starts. If auto-restart is enabled and the last
    job was interrupted (status still 'running'), resume it automatically.
    This covers Heroku's daily dyno restart: the process auto-starts, and an
    in-progress job picks up where it left off without manual intervention."""
    try:
        with state_lock:
            enabled = auto_restart_enabled
        if not enabled or mongo_job is None:
            return
        job = _get_last_job()
        ...
        do_delete = bool(job.get("do_delete", False))
        if not remote_path:
            return
        log.warning(f"♻️ Resume-on-startup: resuming interrupted job for {remote_path!r}")
        try:
            files = await asyncio.get_running_loop().run_in_executor(
                None, rclone_list, remote_path)
        except Exception as e:
            log.error(f"Resume-on-startup: failed to list {remote_path}: {e}")
            return
        if not files:
            log.info("Resume-on-startup: no files left at remote — marking job done.")
            _mark_last_job("done")
            return
        status_msg = await app.send_message(
            DUMP_CHAT_ID,
            f"♻️ **Auto-restart** — resuming interrupted job after restart:\n`{remote_path}`\n"
        ...
        results = await _run_job(status_msg, files, remote_path, mode,
                                 OWNER_ID, do_delete, size_map=size_map)
        await _send_summary(status_msg, results)
    except Exception as e:
        log.error(f"Resume-on-startup error: {e}")
...
if __name__ == "__main__":
    ...
    async def _main():
        await app.start()
        # Resume an interrupted job once, right after startup (covers the
        # Heroku daily dyno restart). Runs in the background so it doesn't
        # block the bot from accepting commands.
        asyncio.create_task(_resume_on_startup())
        await idle()
        await app.stop()
